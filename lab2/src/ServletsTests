...
public class ServletTest {
    ...
    private ConcreteDB db;

    @Before
    public void setUp() throws Exception {
        db = ConcreteDB.getInstance();
        MockitoAnnotations.initMocks(this);
    }

    @Test
    public void doPostAuth() throws Exception {
        when(request.getParameter("login")).thenReturn("Temp");
        when(request.getParameter("pass")).thenReturn("Temp");
        ...
        assertTrue(stringWriter.toString().contains("Tmp"));
        assertTrue(stringWriter.toString().contains("user"));
    ...
    public void doPostAuthZeroLogin() throws Exception {
        ...
        when(request.getRequestDispatcher("/errorPage.jsp")).thenReturn(requestDispatcher);
        new AuthServlet().doPost(request, response);
        verify(request, times(1)).setAttribute("Error","Wrong format of login or password");
    }
    @Test
    ...
    public void doGetUnblock() throws Exception {
        when(request.getParameter("card")).thenReturn("654");

        List<Card> blockedCards = new ArrayList<>();
        Card card = new Card();
        card.setCardNumber("654");
